...
filedir = '/Users/ajeems/Downloads/LU/phase shift files/final ver/sample_15';
...
ds_led = 8e3;   %Spacing between neighboring LEDs 8mm
decimation_led = 1;  %  Use every LED, 
z_led = 2.8e+7;  % distance from the center LED to the object

%%% !!!
dia_led = 8;   % diameter of # of LEDs used in the experiment
Ibk_thresh = 3000; % background dark value


%%% LED grid setup (32x32)
grid_size = 32;       % number of LEDs per dimension
half_grid = grid_size/2;
vled = (1:grid_size) - (half_grid + 0.5);
hled = (1:grid_size) - (half_grid + 0.5);
...
LitCoord = zeros(grid_size);

% Parameters for ring
num_leds = 24;             % total LEDs on ring
radius_mm = 68;            % ring radius in mm (approx physical)
radius_um = radius_mm * 1000;      % convert to µm
radius_led = radius_um / ds_led;   % convert to LED units (grid radius)

% Center coordinates (approx 0,0)
angles = linspace(0, 2*pi, num_leds + 1);
angles(end) = [];  % remove duplicate at 2π

for i = 1:num_leds
    x_pos = radius_led * cos(angles(i));
    y_pos = radius_led * sin(angles(i));

    % Find nearest LED grid points
    [~, x_idx] = min(abs(hled - x_pos));
    [~, y_idx] = min(abs(vled - y_pos));

    if x_idx >= 1 && y_idx >= 1 && x_idx <= grid_size && y_idx <= grid_size
        LitCoord(y_idx, x_idx) = 1;
    end
end

% Add center LED 
CenterCoord = zeros(grid_size);
CenterCoord(half_grid, half_grid) = 1;

% Apply decimation if needed
LitCoord(:, mod(1:grid_size, decimation_led) ~= 0) = 0; % decimation factor
LitCoord(mod(1:grid_size, decimation_led) ~= 0, :) = 0; % decimation factor
...
illumination_na_used = illumination_na(Litidx);
